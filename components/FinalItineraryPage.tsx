  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="text-center mb-8">
        <h2 className="text-4xl font-extrabold text-slate-900 dark:text-white">{t('final.title', { destination: itinerary.destination })}</h2>
        <p className="mt-2 text-slate-600 dark:text-slate-400">{t('final.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {itinerary.daily_plans.map(plan => (
            <div key={plan.day} className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
              <h3 className="text-2xl font-bold text-cyan-600 dark:text-cyan-400 mb-4">{t('final.day', { day: plan.day })}: {plan.title} <span className="text-lg font-normal text-slate-500 dark:text-slate-400">({plan.date})</span></h3>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 sticky top-24">
            <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-4">{t('final.bookedServices')}</h3>
            {bookedServices.guide || bookedServices.vehicle ? (
              <div className="space-y-4">
                {bookedServices.guide && (
                  <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-lg">
                    <p className="font-bold text-slate-700 dark:text-slate-300">{t('final.guide')}</p>
                    <p className="text-slate-600 dark:text-slate-400">{bookedServices.guide.name}</p>
                    <p className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">¥{bookedServices.guide.pricePerDay * itinerary.duration} / {itinerary.duration} {t('itinerary.days')}</p>
                  </div>
                )}
                {bookedServices.vehicle && (
                  <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-lg">
                    <p className="font-bold text-slate-700 dark:text-slate-300">{t('final.vehicle')}</p>
                    <p className="text-slate-600 dark:text-slate-400">{bookedServices.vehicle.name}</p>
                    <p className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">¥{bookedServices.vehicle.pricePerDay * itinerary.duration} / {itinerary.duration} {t('itinerary.days')}</p>
                  </div>
                )}
                <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{t('final.totalCost')}: ¥{totalServiceCost}</p>
                </div>
              </div>
            ) : (
              <p className="text-slate-500 dark:text-slate-400">{t('final.noServices')}</p>
            )}

            <div className="mt-8 text-center text-green-600 dark:text-green-400 flex flex-col items-center">
                <CheckCircleIcon className="h-12 w-12"/>
                <p className="font-bold mt-2">{t('final.ready')}</p>
            </div>
          </div>
        </div>
      </div>
